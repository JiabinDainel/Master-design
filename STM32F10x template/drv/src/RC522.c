...
{                                      // Counter used to clock out the data
    ...
    SPIData = SPIWriteReadData(SPI_2,0xFF);
...
{
	SPIWriteReadData(SPI_2,SPIData);
}
...
void init_rc522(void)
{
    ...
    GPIO_Init(RC522_CS_PIN_INDEX, &GPIO_InitStructure);  

    GPIO_InitStructure.GPIO_Pin = RC522_RST_PIN;  
    GPIO_Init(RC522_RST_PIN_INDEX, &GPIO_InitStructure);  
    //初始化SPI
    SPIInit(SPI_2);
    //初始化RC522
    PcdReset();
